...
const Header = (props) => {
  return (
    <Fragment>
      {/*'classes', referring to what we made up in the import statement. '.header' is the name of the class used in the CSS module file. Refer to the CSS file for comfirmation */}
      <header className={classes.header}>
        <h1>Meal Prep App!</h1>
        {/* <button>Cart</button>. This is replaced with below. A component created to represent the Cart button */}
        <HeaderCartButton onClick={props.onShowCart}/> {/*'onClick' is a custom name for a custom component, NOT the html 'onClick' internal property. The name of the function being passed FROM 'app.js' is called 'onShowCart' (which is custom named by me), so we call through props, that function. */}
      </header>
